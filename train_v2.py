...
def train_RNN(rnn_model, pretrained_cnn, optimizer, trainloader, spoof_labels, criterion, cnn_device, rnn_device,
    ...
    for epoch in range(n_epoch):
        ...
        for i, data in tqdm(enumerate(trainloader, 0)):
            ...
            optimizer.zero_grad()
            outputs_F = rnn_model(F)

            # handle NaN:
            if torch.norm((outputs_F != outputs_F).float()) == 0:
                if spoof_labels[i * 5] == 0:  # all the images in the batch come from the same video
            ...
            if i % 100 == 0:
                print('[RNN: Epoch: %d, Batch: %5d] loss: %.3f' % (epoch + 1, i, running_loss / len(trainloader)))
...
if __name__ == '__main__':
    ...
    train_RNN(rnn_model=rnn_model, pretrained_cnn=cnn_model, optimizer=rnn_optimizer, trainloader=trainloader_D,
              ...
              criterion=criterion, cnn_device=gpu0, rnn_device=gpu1, n_epoch=10)
