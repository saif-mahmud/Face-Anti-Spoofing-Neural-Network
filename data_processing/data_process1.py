            ...
            for user in range(1, 21):
                for file in range(1, 6):
                    ...
                    cap = cv2.VideoCapture(
                        '/Volumes/G-DRIVE mobile USB/Train_files_OULU/Train_files/' + str(phone) + '_' + str(
                            session) + '_' + nom + '_' + str(file) + '.avi')

                    lines = []
                    with open('/Volumes/G-DRIVE mobile USB/Train_files_OULU/Train_files/' + str(phone) + '_' + str(
                            session) + '_' + nom + '_' + str(file) + '.txt', 'r') as f:
                        for i in range(5):
        ...
        for session in range(1, 4):
            for user in range(21, 36):
                for file in range(1, 6):
                    ...
                    if user < 10:
                        nom = '0' + nom
                    cap = cv2.VideoCapture('/Volumes/G-DRIVE mobile USB/dev_files/Dev_files/' + str(phone) + '_' + str(
                        session) + '_' + nom + '_' + str(file) + '.avi')

                    lines = []
                    with open('/Volumes/G-DRIVE mobile USB/dev_files/Dev_files/' + str(phone) + '_' + str(
                            session) + '_' + nom + '_' + str(file) + '.txt', 'r') as f:
                        for i in range(5):
                            lines.append(f.readline())
            ...
            for user in range(36, 56):
                for file in range(1, 6):
                    ...
                    if user < 10:
                        nom = '0' + nom
                    cap = cv2.VideoCapture(
                        '/Volumes/G-DRIVE mobile USB/Test_files_OULU/Test_files/' + str(phone) + '_' + str(
                            session) + '_' + nom + '_' + str(file) + '.avi')

                    lines = []
                    with open('/Volumes/G-DRIVE mobile USB/Test_files_OULU/Test_files/' + str(phone) + '_' + str(
                            session) + '_' + nom + '_' + str(file) + '.txt', 'r') as f:
                        for i in range(5):
                            lines.append(f.readline())
